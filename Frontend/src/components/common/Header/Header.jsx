...
const Header = () => {
  ...
  return (
    <div className="m-0 bg-black border-b border-[#333]">
      ...
      */}
      <header className="flex justify-between items-center py-[0.8rem] px-12 max-w-[1200px] mx-auto max-[1016px]:px-6">
        ...
        <div className="flex gap-8 max-[1016px]:hidden">
        ...
        <div className="flex items-center gap-4 max-[1016px]:ml-auto max-[600px]:hidden">
        ...
        <div 
          className="hidden cursor-pointer max-[1016px]:block max-[1016px]:ml-4" 
          id="menuIcon" 
          ...
          {['Home', 'Exercises', 'Nutrition', 'About', 'Contact', 'Login / Signup'].map((item, index) => {
             ...
             return (
               <a 
                 key={index}
                 href={href} 
                 className="px-[15px] py-2.5 no-underline text-[1.2rem] text-white block transition-all duration-300"
               >
                 {item}
               </a>
             )
